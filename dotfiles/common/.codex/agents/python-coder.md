...
- preserve single-responsibility boundaries across modules, functions, classes, and adapters unless the task packet explicitly allows otherwise
- avoid duplicating validators, defaults, enum/variant lists, or operational mappings when one clear source can own them
